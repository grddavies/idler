...
test_server_warning <- function(input, output, session) {
  idler::set(2, warn_after = 0.5)
}
...
test_ui <- function(request) {
  shiny::fluidPage(idler::use_idler(), shiny::tags$h1("Test"))
}
...
test_that("Users can be warned prior to session timeout", {
  skip_on_cran()
  app <- shinytest::ShinyDriver$new(
    shiny::shinyApp(test_ui, test_server_warning)
  )
  app$waitForShiny()
  app$waitFor("", timeout = 750)
  last_console_log <- tail(app$getDebugLog("shiny_console")$message, n = 1)
  expect_match(last_console_log, "warned", perl = TRUE)
  app$waitFor("", timeout = 1200)
  last_console_log <- tail(app$getDebugLog("shiny_console")$message, n = 1)
  expect_match(last_console_log, "timed out", perl = TRUE)
  app$finalize()
})
